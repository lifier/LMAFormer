...
def flow_loss(frames_list, flow_forwards, flow_backwards):
    t, c, h, w = frames_list.size()

    frames_1 = frames_list[:-1, :, :, :].reshape(-1, c, h, w)
    frames_2 = frames_list[1:, :, :, :].reshape(-1, c, h, w)
    backward_frames = warp(frames_1, flow_forwards.reshape(-1, 2, h, w))
    forward_frames = warp(frames_2, flow_backwards.reshape(-1, 2, h, w))
    forward_loss = F.l1_loss(forward_frames, frames_1)
    backward_loss = F.l1_loss(backward_frames, frames_2)
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, weight_dict, losses, is_sc_block=True, aux_loss=0, aux_loss_norm=0):
        super().__init__()
        self.num_classes = 1
        self.losses = losses
    ...
    def loss_masks(self, outputs, targets, samples, flow_forwards, flow_backwards):
        ...
        flow_loss_ = 0
        for i in range(len(flow_forwards)):
            flow_size = flow_forwards.shape[-2:]
            temp_samples = interpolate(samples, size=flow_size, mode="bilinear", align_corners=False)
            flow_loss_ += flow_loss(temp_samples, flow_forwards[i], flow_backwards[i])/len(flow_forwards)
    ...
    def forward(self, outputs, targets, samples, flow_forwards, flow_backwards):
        ...
        """
        losses = {}
        losses.update(self.loss_masks(outputs, targets, samples, flow_forwards, flow_backwards))
        return losses
